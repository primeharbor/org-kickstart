...
| <a name="input_declarative_policies"></a> [declarative\_policies](#input\_declarative\_policies) | Map of Declarative Policies to create and attach. | <pre>map(object({<br/>    policy_name        = string<br/>    policy_description = string<br/>    policy_json_file   = string<br/>    policy_targets     = optional(list(string), ["Root"])<br/>    policy_vars        = optional(map(string), {})<br/>    do_not_attach      = optional(bool, false)<br/>  }))</pre> | `{}` | no |
